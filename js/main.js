...
// anteprime live nelle card progetto: caricano solo quando entrano nello schermo,
// si scaricano quando escono, per non appesantire la memoria su mobile
(function lazyPreviews(){
  const allLazy = document.querySelectorAll('iframe.lazy-frame');
  const cardOnly = Array.from(allLazy).filter(f => !f.closest('.hud-frame'));
  if(!cardOnly.length) return;

  const isTouch = window.matchMedia('(pointer: coarse)').matches;
  const maxConcurrent = isTouch ? 1 : 3;
  let active = [];
  ...
  const io = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      ...
      if(entry.isIntersecting){
        load(iframe);
        active = active.filter(f => f !== iframe);
        active.push(iframe);
  ...
  cardOnly.forEach(f => io.observe(f));
})();
