...
// dissolvenza morbida per tutte le anteprime (sia native che gestite via JS)
function fadeInWhenLoaded(iframe){
  iframe.addEventListener('load', () => iframe.classList.add('is-loaded'));
}
document.querySelectorAll('.fade-frame').forEach(fadeInWhenLoaded);

// hero HUD cycling (solo se presente in pagina) — carica un solo iframe alla volta,
// e si ferma del tutto quando l'hero non è visibile (niente caricamenti in background)
const frames = document.querySelectorAll('.hud-frame');
const urlEl = document.getElementById('hud-url');
const hud = document.querySelector('.hud');
if(frames.length && hud){
  const urls = Array.from(frames).map(f => f.dataset.url || '');
  let current = 0;
  let heroInterval = null;

  function loadFrame(frame){
    const iframe = frame.querySelector('iframe.lazy-frame');
    if(iframe && !iframe.src && iframe.dataset.src) iframe.src = iframe.dataset.src;
  }
  function unloadFrame(frame){
    const iframe = frame.querySelector('iframe.lazy-frame');
    if(iframe){ iframe.removeAttribute('src'); iframe.classList.remove('is-loaded'); }
  }

  function startCycle(){
    if(heroInterval) return;
    loadFrame(frames[current]);
    heroInterval = setInterval(() => {
      unloadFrame(frames[current]);
...
// le anteprime nelle card progetto usano il lazy loading nativo del browser
// (loading="lazy"): più fluido, senza i "salti" del caricamento/scaricamento manuale
